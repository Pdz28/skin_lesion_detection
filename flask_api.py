...
def process_image():
    ...
    if file and allowed_file(file.filename):
        ...
        model = load_model()
        result_image = detect_skin_disease(model, processed_image)

        # Save the result image
        result_image_path = os.path.join(app.config['RESULT_FOLDER'], 'result_' + secure_filename(file.filename))
        result_image.save(result_image_path)

        return jsonify({"original_image": secure_filename(file.filename), "result_image": 'result_' + secure_filename(file.filename)}), 200
...
def results():
    ...
    return render_template('results.html', username=session['username'], original_image=original_image, result_image=result_image)
